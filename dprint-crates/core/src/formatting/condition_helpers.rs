...
pub fn is_hanging(condition_context: &mut ConditionResolverContext, start_lsil: LineStartIndentLevel, end_lsil: Option<LineStartIndentLevel>) -> Option<bool> {
  let start_indent_level = condition_context.resolved_line_start_indent_level(start_lsil)?;
  let end_indent_level = resolved_end_lsil(condition_context, end_lsil)?;
  return Some(end_indent_level > start_indent_level);

  fn resolved_end_lsil(condition_context: &mut ConditionResolverContext, end_lsil: Option<LineStartIndentLevel>) -> Option<u8> {
    if let Some(end_lsil) = end_lsil {
      condition_context.resolved_line_start_indent_level(end_lsil)
    } else {
...
pub fn are_line_and_columns_equal(condition_context: &mut ConditionResolverContext, start_lc: LineAndColumn, end_lc: LineAndColumn) -> Option<bool> {
  let start_lc = condition_context.resolved_line_and_column(start_lc)?;
  let end_lc = condition_context.resolved_line_and_column(end_lc)?;
...
pub fn is_at_same_position(condition_context: &mut ConditionResolverContext, line_and_col: LineAndColumn) -> Option<bool> {
  let (start_ln, start_col) = condition_context.resolved_line_and_column(line_and_col)?;
